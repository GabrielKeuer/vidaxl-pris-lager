...
def download_benuta_csv():
    """Download CSV fra Benuta med retry-logik (defensiv — supplier-server
    kan have udfald). 3 forsøg med backoff 60s/120s. Timeout 60s.
    """
    log("📥 Downloading Benuta CSV...")
    last_err = None
    for attempt in range(1, 4):
        try:
            response = requests.get(BENUTA_CSV_URL, timeout=60)
    ...
    log(f"❌ Could not download Benuta CSV after 3 attempts")
    sys.exit(1)
